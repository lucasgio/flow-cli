...
class WebUseCase {
  ...
  WebLogEntry _parseWebLogLine(String line, String source) {
    ...
    if (line.contains('[ERROR]') || line.contains('ERROR:') || line.contains('Exception:')) {
      ...
      level = 'debug';
    } else if (line.contains('[INFO]') || line.contains('INFO:') || line.contains('Serving at')) {
      level = 'info';
    }
    
    // Clean up common Flutter web prefixes
    message = message
      .replaceAll(RegExp(r'^\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3}\] '), '')
  ...
  Future<Map<String, dynamic>> getPerformanceStats(WebServerSession session) async {
    try {
      return {
  ...
  Future<bool> buildWeb({
    ...
    try {
      ...
      if (result.exitCode == 0) {
        ...
        CliUtils.printError('Web build failed with exit code: ${result.exitCode}');
        print(result.stderr);
        return false;
      }
...
''';
  ...
  Future<Map<String, dynamic>> analyzeWebBuild(String outputDir) async {
    try {
      ...
      if (assetCount > 100) {
        recommendations.add('Large number of assets detected - consider optimizing images');
      }
      ...
      if (await mainJsFile.exists()) {
        ...
        if (mainJsSize > 2 * 1024 * 1024) { // > 2MB
          recommendations.add('Main Dart JS file is large - enable tree shaking');
        }
      }
  ...
  Future<bool> deployToFirebase(String outputDir, String? client) async {
    try {
      ...
      if (firebaseResult.exitCode != 0) {
        CliUtils.printError('Firebase CLI not found. Install it first: npm install -g firebase-tools');
        return false;
      }
  ...
  Future<bool> deployToNetlify(String outputDir, String? client) async {
    try {
      ...
      if (netlifyResult.exitCode != 0) {
        CliUtils.printError('Netlify CLI not found. Install it first: npm install -g netlify-cli');
        return false;
      }
  ...
  Future<bool> deployToVercel(String outputDir, String? client) async {
    try {
      ...
      if (vercelResult.exitCode != 0) {
        CliUtils.printError('Vercel CLI not found. Install it first: npm install -g vercel');
        return false;
      }
  ...
  Future<bool> deployToGitHubPages(String outputDir, String? client) async {
    try {
      ...
      final gitResult = await Process.run('git', ['status'], workingDirectory: projectPath);
      ...
      for (final command in commands) {
        ...
        if (result.exitCode != 0 && !command.contains('checkout')) {
          CliUtils.printError('GitHub Pages deployment failed at: ${command.join(' ')}');
          return false;
        }
      }
  ...
  Future<bool> deployToAWS(String outputDir, String? client) async {
    try {
      ...
      if (awsResult.exitCode != 0) {
        CliUtils.printError('AWS CLI not found. Install and configure it first.');
        return false;
      }
      
      CliUtils.printInfo('AWS S3 deployment requires bucket configuration.');
      CliUtils.printInfo('This would typically use: aws s3 sync $outputDir s3://your-bucket --delete');
  ...
  Future<bool> deployToCustomServer(String outputDir, String? client) async {
    try {
      CliUtils.printInfo('Custom server deployment requires FTP/SFTP configuration.');
      CliUtils.printInfo('You can use tools like rsync, scp, or FTP clients to upload:');
