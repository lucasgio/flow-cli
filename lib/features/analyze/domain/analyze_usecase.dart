...
class AnalyzeUseCase {
      ...
      if (await pubspecFile.exists()) {
        ...
        if (flutter != null) {
          if (flutter['uses-material-design'] != true) {
            recommendations.add('Consider enabling Material Design for better performance');
          }
          
          if (flutter['generate'] != true) {
            recommendations.add('Enable code generation for better build performance');
          }
        }
  ...
  Future<void> saveReport(Map<String, dynamic> results, String outputFile) async {
    try {
  ...
  void _checkDependencyOptimizations(Map dependencies, List<String> recommendations) {
  ...
  Future<void> _checkAndroidOptimizations(List<String> recommendations) async {
    try {
      final projectPath = _configService.projectPath!;
      final buildGradle = File(path.join(projectPath, 'android', 'app', 'build.gradle'));
      
      if (await buildGradle.exists()) {
        final content = await buildGradle.readAsString();
        
        if (!content.contains('shrinkResources true')) {
          recommendations.add('Enable resource shrinking in Android build.gradle');
        }
        
        if (!content.contains('minifyEnabled true')) {
          recommendations.add('Enable code minification in Android build.gradle');
        }
      }
    } catch (e) {
  ...
  Future<void> _checkIOSOptimizations(List<String> recommendations) async {
    try {
      final projectPath = _configService.projectPath!;
      final infoPlist = File(path.join(projectPath, 'ios', 'Runner', 'Info.plist'));
      
      if (await infoPlist.exists()) {
        final content = await infoPlist.readAsString();
        
        if (!content.contains('NSAppTransportSecurity')) {
          recommendations.add('Consider configuring App Transport Security for iOS');
        }
      }
    } catch (e) {
      // Ignore errors for optional checks
  ...
  Future<void> _checkFlutterOptimizations(List<String> recommendations) async {
    try {
      ...
      if (await libDir.exists()) {
        recommendations.add('Review widget constructors for const optimization opportunities');
      }
      
      // Check for build methods that are too complex
      recommendations.add('Consider breaking down complex build methods into smaller widgets');
  ...
  Future<void> _checkDartPerformanceIssues(String projectPath, List<String> issues) async {
    try {
      final libDir = Directory(path.join(projectPath, 'lib'));
      if (await libDir.exists()) {
  ...
  Future<void> _checkImageOptimization(String projectPath, List<String> issues) async {
    try {
      final assetsDir = Directory(path.join(projectPath, 'assets'));
      if (await assetsDir.exists()) {
        final imageFiles = await assetsDir.list(recursive: true)
  ...
  Future<void> _checkDependencyPerformance(String projectPath, List<String> issues) async {
    try {
      final pubspecFile = File(path.join(projectPath, 'pubspec.yaml'));
      if (await pubspecFile.exists()) {
  ...
  Future<void> _checkSecurityIssues(String projectPath, List<String> issues) async {
    try {
      final pubspecFile = File(path.join(projectPath, 'pubspec.yaml'));
      if (await pubspecFile.exists()) {
        final content = await pubspecFile.readAsString();
        ...
        if (content.contains('debug:')) {
          issues.add('Debug configurations found - ensure they are not in production');
        }
      }
    } catch (e) {
      // Ignore errors for optional checks
