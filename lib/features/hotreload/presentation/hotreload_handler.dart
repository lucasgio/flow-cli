...
class HotReloadHandler {
  ...
  Future<void> handle(List<String> args) async {
    final parser = ArgParser()
      ...
      ..addOption('log-level', help: 'Log level filter (all, debug, info, warning, error)', defaultsTo: 'all')
      ..addFlag('no-sound', help: 'Disable sound notifications', negatable: false)
      ..addFlag('help', abbr: 'h', help: 'Show help for hotreload command', negatable: false);
    ...
    try {
      ...
      CliUtils.printInfo('Target device: ${targetDevice.name} (${targetDevice.platform})');
  ...
  Future<DeviceModel?> _selectTargetDevice(String? deviceId) async {
    ...
    final deviceNames = onlineDevices.map((d) => '${d.name} (${d.platform}) - ${d.id}').toList();
  ...
  Future<void> _startHotReloadSession({
    ...
    try {
      ...
      logSubscription = _hotReloadUseCase.getLogStream(hotReloadSession)
        .listen((logEntry) {
  ...
  Future<void> _performHotReload(HotReloadSession session, bool soundEnabled) async {
    CliUtils.clearLine();
    ...
    if (success) {
      CliUtils.printSuccess('⚡ Hot reload completed in ${stopwatch.elapsedMilliseconds}ms');
  ...
  Future<void> _performHotRestart(HotReloadSession session, bool soundEnabled) async {
    CliUtils.clearLine();
    ...
    if (success) {
      CliUtils.printSuccess('🔄 Hot restart completed in ${stopwatch.elapsedMilliseconds}ms');
      if (soundEnabled) _playSuccessSound();
    } else {
  ...
  Future<void> _showDeviceInfo(DeviceModel device) async {
    ...
    if (device.architecture != null) print('Architecture: ${device.architecture}');
    CliUtils.printSeparator();
    print('');
  }
