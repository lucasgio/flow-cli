...
class DeviceUseCase {
  ...
  Future<bool> runOnDevice(DeviceModel device, {String? client, List<String>? additionalArgs}) async {
    try {
      final flutterPath = _configService.flutterPath!;
  ...
  Future<bool> uninstallApp(DeviceModel device, {String? client}) async {
    try {
      ...
      if (device.platform == 'android') {
        ...
        CliUtils.printError('Uninstall not supported for platform: ${device.platform}');
        return false;
      }
  ...
  Future<String?> _getAppPackageName(String? client) async {
    try {
      ...
      if (client != null) {
        ...
        final configPath = path.join(projectPath, 'assets', 'configs', client, 'config.json');
  ...
  Future<bool> _uninstallAndroidApp(DeviceModel device, String packageName) async {
    try {
      final result = await Process.run(
        'adb',
  ...
  Future<bool> _uninstallIOSApp(DeviceModel device, String packageName) async {
    try {
      ...
      CliUtils.printWarning('iOS app uninstall requires manual removal or additional tools');
      return false;
    } catch (e) {
      return false;
    }
